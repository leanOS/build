...
"leanremote: Add a git remote for matching LEAN repository"
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the LEAN github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var LEAN_VERSION)
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.lean.device=$LEAN_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $LEAN_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $LEAN_FIXUP_COMMON_OUT ]; then
